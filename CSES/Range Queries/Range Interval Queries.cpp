...
private:
    ...
    node calcDifBetween(node v1, node v2){ // Change
        if(v1.val == neutro.val) return neutro;
        return {v2.val - v1.val, -1, -1};
    }

    node calcSolapeBetween(node a, node b){ // Change
        if(a.val == neutro.val) return b;
        if(b.val == neutro.val) return a;
...
void solver(){
    ...
    for0(i,sz(a)) a[i] = lb(all(aux), a[i]) - aux.begin();
    ...
    for0(i,q){
        int v1, v2, l, r; cin>>v1>>v2>>l>>r; v1--;
        l = lb(all(aux), l) - aux.begin();
        r = ub(all(aux), r) - aux.begin();
        ll ans = st.calcBetween(v1, v2, l, r).val;
        cout<<(ans == LLONG_MAX - 1 ? 0 : ans)<<endl;
    }
}
