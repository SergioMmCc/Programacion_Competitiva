...
const ll maxn = 300005, inf = 2e9;
ll components;
vector<ll> leader(maxn), sizen(maxn), len(maxn);
...
ll join(ll u, ll v, ll i){
    ...
    else if(len[u] == len[v]) ans = i;
    return ans;
}

void updateAllLeaders(ll n){
    for(ll i = 1; i <= n; i++)
        find(i);
}
