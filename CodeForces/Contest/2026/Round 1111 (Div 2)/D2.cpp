...
private:
    ...
    node neutro = {LLONG_MAX - 1, LLONG_MIN + 1, 0LL}; // Change

    node calcOp(node a, node b, ll len){ // Change
        if(a.mini == neutro.mini) return b;
        if(b.mini == neutro.mini) return a;
        node ans;
        ans.mini = min(a.mini, b.mini);
        ans.maxi = max(a.maxi, b.maxi);
        if(a.maxi > b.mini) ans.val = len;
        else ans.val = max(a.val, b.val);
        return ans;
    }

    void update(int pos, ll val, int v, int tl, int tr){ // O(lg(n))
        if(tr - tl == 1){
            tree[v] = {val, val, 0}; // Change
            return;
        }
        ...
        tree[v] = calcOp(tree[2*v + 1], tree[2*v + 2], tm-tl);
    }
    ...
    node calc(int l, int r, int v, int tl, int tr){ // Change si walking on segment tree
        ...
        return calcOp(m1, m2, tm-tl);
    }

    void build(vl& a, int v, int tl, int tr){ // O(n)
        if(tr - tl == 1){
           if(tl < sz(a)) tree[v] = {a[tl], a[tl], 0}; // Change
           return;
        }
        int tm = (tr + tl) / 2;
        build(a, 2*v + 1, tl, tm);
