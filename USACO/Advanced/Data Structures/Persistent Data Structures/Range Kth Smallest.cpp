...
class segTree {
private:
    node neutro = {INT_MIN + 1, -1, -1}; // Change
    ...
    // [l, r) y [v1, v2]
    int calcBetween(int v1, int v2, int l, int r, int k, int tl, int tr){
        if(tl >= r || l >= tr) return neutro.val;
        if(tr - tl == 1) return tl;
        int tm = (tl + tr) / 2;
        int dif = calcDifBetween(tree[tree[v1].l], tree[tree[v2].l]).val;
        if(dif > k) return calcBetween(tree[v1].l, tree[v2].l, l, r, k, tl, tm);
        k -= dif;
        return calcBetween(tree[v1].r, tree[v2].r, l, r, k, tm, tr);
    }
...
public:
    ...
    int calcBetween(int v1, int v2, int l, int r, int k){
        return calcBetween(roots[v1], roots[v2], l, r, k, 0, n);
    }
...
void solver(){
    ...
    while(q--){
        int l, r, k; cin>>l>>r>>k;
        int ans = st.calcBetween(l, r, 0, n, k);
        cout<<inv[ans]<<endl;
    }
}
