...
void solver(){
    int n; cin>>n;
    MCMF graph(2*n + 3);
    graph.addEdge(2*n + 1, 2*n + 2, 4, 0);
    vector<int> a(n);
    for(int i = 0; i < n; i++){
        cin>>a[i];
        graph.addEdge(2*n, 2*i, 1, -1);
        graph.addEdge(2*i + 1, 2*n + 1, 1, 0);
        graph.addEdge(2*i, 2*i + 1, 1, 0);
    }

    for(int i = 0; i < n - 1; i++){
        for(int j = i + 1; j < n; j++){
            if(a[i] - 1 == a[j] || a[i] == a[j] - 1 || (a[i] % 7) == (a[j] % 7)) graph.addEdge(2*i + 1, 2*j, 1, -1);
        }
    }

    graph.setpi(2*n);
    cout<<-1 * graph.maxflow(2*n, 2*n + 2).se<<endl;
}
