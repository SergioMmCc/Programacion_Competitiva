...
private:
    ...
    node neutro = {LLONG_MAX - 1, 0}; // Change

    node calcOp(node a, node b){ // Change
        if(a.a == neutro.a) return b;
        if(b.a == neutro.a) return a;
        node ans = {(b.a * a.a) % mod, (((b.a * a.b) % mod) + b.b) % mod};
        return ans;
    }

    void update(int pos, node val, int v, int tl, int tr){ // O(lg(n))
        if(tr - tl == 1){
            tree[v] = val; // Change
            return;
        }
    ...
    void build(vector<node>& a, int v, int tl, int tr){ // O(n)
        if(tr - tl == 1){
...
void solver(){
    ...
    while(q--){
        int op; cin>>op;
        if(!op){
            int idx; ll m, b; cin>>idx>>m>>b;
            st.update(idx, {m, b});
        }
        else{
            int l, r; ll x; cin>>l>>r>>x;
            node ans = st.calc(l, r);
            cout<<(((ans.a * x) % mod) + ans.b) % mod<<endl;
        }
    }
}
